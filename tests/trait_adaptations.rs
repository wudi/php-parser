use bumpalo::Bump;
use php_parser_rs::ast::{ClassMember, TraitAdaptation, TraitMethodRef, Stmt};
use php_parser_rs::lexer::Lexer;
use php_parser_rs::parser::Parser;

#[test]
fn parses_trait_use_adaptations() {
    let code = "<?php class C { use A, B { A::foo insteadof B; B::foo as public bar; foo as baz; } }";
    let arena = Bump::new();
    ...
    let trait_use = members
        .iter()
        .find_map(|m| match m {
            ClassMember::TraitUse { traits, adaptations, .. } => Some((traits, adaptations)),
            _ => None,
        })
    ...
    match adaptations[0] {
        TraitAdaptation::Precedence { method: TraitMethodRef { trait_name: Some(_), .. }, insteadof, .. } => {
    ...
    match adaptations[1] {
        TraitAdaptation::Alias { visibility, alias, .. } => {
    ...
    match adaptations[2] {
        TraitAdaptation::Alias { method: TraitMethodRef { trait_name: None, .. }, alias, .. } => {
            assert!(alias.is_some());
        }
