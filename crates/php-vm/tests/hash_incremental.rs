...
fn test_hash_init_basic() {
    ...
    let ctx_handle = php_vm::builtins::hash::php_hash_init(&mut vm, &[algo_handle])
...
fn test_hash_update_single_chunk() {
    ...
    let data_handle = vm.arena.alloc(Val::String(Rc::new(b"Hello, World!".to_vec())));
    php_vm::builtins::hash::php_hash_update(&mut vm, &[ctx_handle, data_handle])
        .expect("hash_update failed");
    
    let result_handle = php_vm::builtins::hash::php_hash_final(&mut vm, &[ctx_handle])
