...
fn test_class_constants_basic() {
    ...
    if let Val::Array(map) = result {
        assert_eq!(map.len(), 4);
        // A::X = 10
        assert_eq!(vm.arena.get(*map.get_index(0).unwrap().1).value, Val::Int(10));
        // A::Y = 20
        assert_eq!(vm.arena.get(*map.get_index(1).unwrap().1).value, Val::Int(20));
        // B::X = 11
        assert_eq!(vm.arena.get(*map.get_index(2).unwrap().1).value, Val::Int(11));
        // B::Y = 20 (inherited)
        assert_eq!(vm.arena.get(*map.get_index(3).unwrap().1).value, Val::Int(20));
    } else {
...
fn test_class_constants_visibility_access() {
    ...
    if let Val::Array(map) = result {
        assert_eq!(map.len(), 5);
        assert_eq!(vm.arena.get(*map.get_index(0).unwrap().1).value, Val::Int(3)); // PUB
        assert_eq!(vm.arena.get(*map.get_index(1).unwrap().1).value, Val::Int(1)); // getPriv
        assert_eq!(vm.arena.get(*map.get_index(2).unwrap().1).value, Val::Int(2)); // getProt
        assert_eq!(vm.arena.get(*map.get_index(3).unwrap().1).value, Val::Int(2)); // getParentProt
        assert_eq!(vm.arena.get(*map.get_index(4).unwrap().1).value, Val::Int(2)); // getSelfProt
    } else {
        panic!("Expected array");
    }
}
