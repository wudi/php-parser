    ...
    if let Val::String(s) = &val.value {
        if let Some(sym) = vm.context.interner.find(s) {
            if let Some(def) = vm.context.classes.get(&sym) {
                // class_exists returns true for classes, but false for interfaces and traits
                return Ok(vm.arena.alloc(Val::Bool(!def.is_interface && !def.is_trait)));
            }
        }
    }
    ...
    let class_sym = match &val.value {
        Val::String(s) => {
            if let Some(sym) = vm.context.interner.find(s) {
                ...
                // PHP behavior: trigger error but return false/null? 
                // For now let's return empty array or error.
                // PHP 8: Error if class not found.
