...
use crate::ast::{Stmt, StmtId, Expr, ExprId, Param, ClassMember, AttributeGroup, PropertyHook, PropertyHookBody, ClassConst, TraitMethodRef, Arg, Type, Name, TraitAdaptation};
...
impl<'src, 'ast> Parser<'src, 'ast> {
    pub(super) fn parse_class(&mut self, attributes: &'ast [AttributeGroup<'ast>], modifiers: &'ast [Token]) -> StmtId<'ast> {
    ...
    pub(super) fn parse_anonymous_class(&mut self) -> (ExprId<'ast>, &'ast [Arg<'ast>]) {
    ...
    pub(super) fn parse_interface(&mut self, attributes: &'ast [AttributeGroup<'ast>]) -> StmtId<'ast> {
        let start = if let Some(first) = attributes.first() {
            first.span.start
        } else {
            self.current_token.span.start
    ...
    pub(super) fn parse_function(&mut self, attributes: &'ast [AttributeGroup<'ast>]) -> StmtId<'ast> {
        let start = if let Some(first) = attributes.first() {
            first.span.start
        } else {
            self.current_token.span.start
